class User
  ...
  #unfortunately the :unique => true auto validation works in scope of the subclass
  #don't know how to bypass that -- custom validator works globally
  def check_login_is_unique
    if User.first(:conditions => {:login => self.login})
