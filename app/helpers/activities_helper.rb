module ActivitiesHelper
    ...
    calendar_table(:year => year, :month => month, :first_day_of_week => 1, :owner_type => owner_type) do |date|
      ...
      if activities_for_today
        ...
        for activity in shown_activities
          html << render(:partial => 'activity', :locals => {:activity => activity})
        end
  ...
  def calendar_table(options = {}, &block)
    ...
    cal << link_to("&laquo; Previous".html_safe, prev_url, :id => "previous_month")
    ...
    unless @next_month.nil? && @next_year.nil?
      cal << link_to("Next &raquo;".html_safe, next_url, :id => "next_month")
    end
  ...
  def day_url(criteria)
    CGI.escapeHTML(day_activities_path(:search_criteria => criteria)+"&width=600&height=500")
  end

  def prev_day_url
    criteria = params[:search_criteria].clone
