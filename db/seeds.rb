...
unless admin = Employee.first(:login => "admin")
  ...
  admin = Employee.create(
    :name => "Admin Adminiusz",
    :login => "admin",
    :password => pass,
    :password_confirmation => pass,
    :email => "admin@tt666.com",
    :role => developer,
    :admin => true
  )
end
...
Project.all.each do |project|
  ...
  unless HourlyRate.all.any? { |hr| hr.project == project }
    { pm => 888, developer => 777, tester => 666 }.each do |role, rate|
      HourlyRate.create(
        :operation_author => admin,
        :project => project,
        :role => role,
        :currency => Currency.first,
        :takes_effect_at => DateTime.now,
        :value => rate
      )
    end
  end
