...
describe SearchCriteria do
  before(:all) do
    @client1 = fx(:orange) #Client.gen
    @client2 = fx(:apple) ##Client.gen
    @client3 = fx(:banana) #Client.gen
    ...
    @role1 = fx(:developer) #Role.gen
    @role2 = fx(:tester) #Role.gen
    @admin = fx(:admin) #Employee.gen(:admin)
    @dev1 = fx(:jola) #Employee.gen(:role => @role1)
    @dev2 = fx(:misio) #Employee.gen(:role => @role1)
    @dev3 = fx(:stefan) #Employee.gen(:role => @role2)
    @client_user1 = fx(:orange_user1) #ClientUser.gen(:client => @client1)
    @client_user2 = fx(:apple_user1) # ClientUser.gen(:client => @client2)
    @client_user3 = fx(:banana_user1)# ClientUser.gen(:client => @client3)
